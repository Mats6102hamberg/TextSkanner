...
export function maskSensitiveData(text: string): string {
  return text
    .replace(/\b\d{6}[- ]?\d{4}\b/g, "[MASKERAT PERSONNUMMER]")
    .replace(/\b(?:\+46|0)\d{7,12}\b/g, "[MASKERAT TELEFONNUMMER]")
    .replace(/[a-zA-Z0-9._%+-]+@[a-zA-Z0-9.-]+\.[A-Za-z]{2,}/g, "[MASKERAD E-POST]")
    .replace(/\b[A-Za-zÅÄÖåäö]+(?:gatan|vägen|gränd|gata)\s*\d+[A-Za-z]?\b/gi, "[MASKERAD ADRESS]")
